...
var titleState  = {
  create: function(){
    ...
    // Title text
    adventure = game.add.bitmapText(0, game.world.height - 80, 'vapeBit', "An h3h3 Adventure", 16);
    adventure.x = game.world.width / 2 - adventure.width / 2;

    // Start text
    start = game.add.bitmapText(0, game.world.height - 30, 'vapeBit', "Press any key to start", 16);
    start.x = game.world.width / 2 - start.width / 2;
  },
  update: function(){
    // Call the next state
...
var level01State = {
  create: function(){
    ...
    music.addMarker('loop', 0, 64, .125, true);
    music.play('loop');
    ...
    graphics.drawRect(0, 0, game.world.width, 13);
  ...
  update: function(){
    ...
    game.physics.arcade.collide(player, deskBounds, bumpDesk);
    ...
    // Stop animation if no move control is down
...
// Fix PC
function fixPC(thisPCIndex, type) {
  if(type === 'pass') {
  ...
  if(fixedCount % 10 === 0 && powerups.children.length === 0 && !poweredUp) {
    game.time.events.add(Phaser.Timer.SECOND * dropDelay, function() {
      dropPowerup('coffee', thisPCIndex);
    });
  }
}
...
var gameOverState = {
  create: function(){
    ...
    btnX = game.world.width / 2 - 32;
    btnY = game.world.height / 2 + 8
